...
**Dernière mise à jour :** 23 juin 2026 (J+13 run #3 — **Cap. C20 `rock_canvas`** (la paroi à peindre) : 20ᵉ capacité agent et la **2ᵉ brique de l'axe SYMBOLIQUE / du dessin** (le 5ᵉ pilier d'émergence). C18 a livré le **pigment** (la matière de la marque) ; C20 livre son pendant — le **support** (la paroi qui *tient* la marque). **Non-fire** (D9 1 → 0 après le fire-based C19 — alternance honorée, diversifie hors du cluster métallurgique C17/C19), **non mutant** (D10 gelé — `canvas_preview`/`paint_outcome` ne touchent pas la géologie). **COMBLE LE TROU L1 SOUS L'ART L4** : `engine.art_discovery` (L4 Feedback, Wave 13) modélise déjà l'**acte** de dessin (pigment + surface + N traits → empreinte → archétype émergent) avec un dico **abstrait** `PAINTABLE_SURFACES` (`bedrock_calcite` 0,95…), mais **rien ne rendait perceptible, par lieu, QUELLE paroi est là ni si une marque y DURE** ; C20 le **fonde** dans la géologie réelle — pont **L1↔L4** : `CALCITE_ADHESION` byte-égale à `art_discovery.PAINTABLE_SURFACES["bedrock_calcite"]` (verrouillé par test, sans importer la couche L4 → pas d'inversion de dépendance). **COMPOSE C6 `limestone_outcrop`** (la paroi carbonatée + ses états d'altération) ; **aucun nouveau tell** (D8 par composition, **14ᵉ** après C7→C19 ; `PY_TO_RUST` reste 15, hors glob `*_outcrop.py`). **Une vérité de substrat, deux lectures** : C6 calculait l'altération pour la **pierre de taille** (`dressable_now` : se dresse-t-elle en blocs ?) ; C20 relit la **même** vérité (`weather_state` ∈ {SOUND, KARST, FROST}) pour une **autre** question — une marque peinte y **dure**-t-elle ? **Physique calculée** (veille archéologie pariétale 2026-06-23) : deux axes orthogonaux — **adhérence** (porosité du carbonate : craie/calcaire pur poreux 0,95 ≥ calcaire commun 0,92 > marbre dense recristallisé 0,70 ; *« the porous limestone substrate lets mineral pigments bond »*) × **persistance** (stabilité du site : SAINE → **voile de calcite** qui scelle la marque, ~17 000 ans à Lascaux ; KARST/FROST → écaillage) ; `durability = adhésion × persistance`, une marque dure ssi ≥ `MIN_DURABLE`. **Mensonge rendu visible #11** (la belle paroi qui ne tient pas la marque — *climat-driven, pendant exact de C15*) : la **même** falaise blanche conspicue (adhérence forte — c'est du calcaire) **ment** quand elle est instable (KARST dissolution / FROST gel) → persistance effondrée → la marque prend… puis **s'écaille** (`holds_lasting_mark` False). « Looks markable ≠ holds a lasting mark ». 2ᵉ moitié du mensonge : un pigment dont la couleur **épouse** le mur est une vraie peinture mais **invisible** (`mark_visibility`). Le **geste** (tracer, la forme, le sens) reste **émergent** (c'est `art_discovery` L4 qui l'enregistre). **« Le monde ne ment jamais »** : cue ⇒ carbonate C6 (même matériau) ; `holds_lasting_mark` ⟺ `durability` ≥ seuil ; `surface_key == bedrock_calcite`. Monde réel **sans injection** : seed `0xC1A7` (carbonaté tempéré sec, le même que C6) → **144/144 parois SAINES durables** (best_durability 0,855) ; seed `0xFE11` (même carbonate en climat humide) → **144/144 parois KARST dont la marque s'écaille** (durability 0,237) — le mensonge climat-driven émergent. 22 tests + smoke `p152` 8/8, pytest **803/803** (+22 ; 781 avant), ruff clean, axe symbolique 1→2, global ~81,1 %. Veille J+13 run #3 (5 axes) → **0 combo externe** (tous gated) ; combo interne C6 × archéologie pariétale. Détail veille : [`docs/veille/2026-06-23_VEILLE_rock_canvas.md`](docs/veille/2026-06-23_VEILLE_rock_canvas.md). **Antérieur J+13 run #2 — Cap. C19 `bloom_forging`** (le cinglage de la loupe) : 19ᵉ capacité agent, la **6ᵉ transformation et la 3ᵉ MÉTALLURGIQUE** — elle **ferme la chaîne opératoire du fer**. **Exécute la reco `R-J12r3-2` de l'audit J+12** (« la forge de consolidation ferme la chaîne du fer — sans nouveau tell, sans feu nouveau »). C17 `iron_bloomery` a réduit le minerai en **loupe** spongieuse (`requires_forging`/`is_solid_bloom` toujours True) en différant honnêtement *« le martelage de consolidation reste émergent »* ; C19 le **RÉALISE** : marteler la loupe **à chaud** pour **expulser la scorie** de fayalite et la **consolider** en **fer forgé** (wrought iron). **Fire-based** (la forge à chaud est **physiquement obligatoire** — aucun travail à froid ne chasse la scorie d'une loupe) → **D9 0 → 1** après le non-feu C18 : **alternance** propre, pas un treadmill. **Non mutant** (D10 gelé — `consolidate_bloom` transforme un **produit tenu** (la loupe de `bloom_at`), pas la géologie ; comme la trempe C8 / cuisson C9). **COMPOSE** C17 `iron_bloomery` (la loupe, elle-même C12 four ≥1200 °C × C1 chapeau de fer) ; **réutilise le seuil `fd.IRON_BLOOMERY_TEMP_C`** comme `SLAG_EXPULSION_TEMP_C` (le régime où la fayalite coule — un feu nu ≤850 °C est trop froid) ; **aucun nouveau tell** (D8 par composition, **13ᵉ** après C7→C18 ; `PY_TO_RUST` reste 15, hors glob `*_outcrop.py`). **Physique calculée** (veille archéométrie de la consolidation 2026-06-23) : travail à l'état **SOLIDE** — le fer ne **FOND jamais** (`melted` toujours False ; 1538 °C hors d'atteinte → fer forgé, **jamais** fonte/haut-fourneau, différés honnêtement) ; chaque chaude expulse une fraction **géométrique** de la scorie piégée (`EXPEL_PER_HEAT`) et **consolide** plus (`CONSOLIDATION_PER_HEAT`, saturant) mais **brûle** du fer en **battitures** FeO (`SCALE_LOSS_PER_HEAT`) ; **conservation Fe** stricte `wrought + scale + crack == bloom_iron`. **Mensonge rendu visible #10** (le fer du chapeau pyriteux **se brise sous le marteau** — pendant, à l'étape suivante, de l'inversion à 5 voies de C17) : la loupe d'**oxyde** (hématite) se consolide **saine** ; la loupe **red-short** (pyrite — le **FeS** aux joints de grain fond sous la chaleur de forge → phase liquide intergranulaire) **se fissure** à chaud (*hot-shortness*) → rendement de fer forgé **effondré**, santé plafonnée `RED_SHORT_SOUNDNESS_CEIL`. « Le chapeau de fer a livré du fer (C17)… qui ne se forge pas (C19) ». `best_forge_site_near` enseigne cingle la loupe d'oxyde, méfie-toi de la pyrite. **« Le monde ne ment jamais »** : cue ⇒ loupe C17 (même minéral) ; `cracked` ⟺ `red_short` ; `melted` toujours False. `forge_preview`/`consolidate_bloom` non mutants (géologie jamais touchée → D10 gelé). Monde réel seed `0x42` (continent ferreux, le même que C17/C18) ancré **déterministe sans injection** : **78/144 sites de forge = 47 oxyde (fer forgé sain, ~8,6 kg/10 kg loupe) + 31 pyrite (red-short, fissurée, ~4,3 kg)**, **1:1 avec les 78 loupes C17**, 0 violation, best_soundness 0,875. 23 tests + smoke `p151` 8/8, pytest **781/781 (+1 skip)** (+23 ; 758 avant), ruff clean, métallurgie 82→83, global ~81,0 %. Veille J+13 run #2 (5 axes) → **0 combo externe** (Bevy/multi-agent LLM/ML-KEM/Neo4j/mémoire agent tous gated) ; combo interne C17×archéométrie de la consolidation. Détail veille : [`docs/veille/2026-06-23_VEILLE_bloom_forging.md`](docs/veille/2026-06-23_VEILLE_bloom_forging.md). **Antérieur J+13 (run #1) — Cap. C18 `ochre_grinding`** (l'ocre broyée) : 18ᵉ capacité agent et le **9ᵉ OPÉRATEUR ORTHOGONAL** — *broyer* (grind/triturate) — la **1ʳᵉ avancée de l'axe symbolique** (le pigment, substrat du dessin — 5ᵉ pilier d'émergence, immobile depuis J+0). **Exécute la reco `R-J12r3-1` de l'audit J+12** (« prochaine cap orthogonale OU non-fire ») : C17 *fire-based* (D9 → 1), C18 **rompt à nouveau vers le non-feu** → **D9 = 0** (alternance anti-treadmill honorée). Verbe **broyer** : réduire la **terre rouille** du chapeau de fer (C1) → pigment d'oxyde de fer (ocre **rouge** hématite / **noir** magnétite). **Pendant orthogonal de C17 sur la MÊME matière** (le gossan C1) : C17 réduit **à chaud → métal** (mutant) ; C18 broie **à froid → pigment** (non mutant) — une lecture du monde, deux civilisations. **COMPOSE** C1 `surface_mineralization` (gossan) × catalogue (`category`, `yields_per_kg_ore["Fe"]`) ; **aucun nouveau tell** (D8 par composition, **12ᵉ** après C7→C17 ; `PY_TO_RUST` reste 15, hors glob `*_outcrop.py`). **Physique calculée** (veille archéochimie du pigment 2026-06-23) : l'ocre EST l'oxyde de fer terreux — **seuls les oxydes** donnent un pigment **stable, lightfaste** (hématite `Fe₂O₃` → rouge ; magnétite `Fe₃O₄` → noir, chroma moindre) ; force colorante ∝ **finesse de broyage**, plafonnée à la chroma intrinsèque ; ocre brûlée (goethite → hématite, 250–400 °C) **différée honnêtement** (fire-based, future cap). **Mensonge rendu visible #9** (le chapeau de fer ment AUSSI au peintre — pendant orthogonal de l'inversion à 5 voies de C17) : le **même** tell rouille coiffe l'**oxyde** (pigment réel) MAIS aussi le **sulfure** pyrite (`FeS₂` riche en fer, rouille → **0 pigment terreux stable**) ET le **non-fer** galène/sphalérite (plomb/zinc → **pas d'ocre**). « Rouille ≠ rouge » : `best_ochre_site_near` enseigne broie l'oxyde, ignore le reste. **« Le monde ne ment jamais »** : minéral == C1 gossan ; `is_pigment` ⟺ oxyde de fer ; le tell de **surface** reste la rouille C1 (la couleur de la **poudre** est une sortie module, **pas** un tell cross-langage → aucune obligation palette Rust). `grind_ochre_at` **non mutant** (géologie jamais touchée → **D10 gelé**, comme C14/C15/C16). Monde réel seed `0x42` (continent ferreux, le même que C17) ancré **déterministe sans injection** : **108/144 sites = 59 hématite (ocre rouge utilisable) + 49 pyrite (rouille mensongère, 0 pigment)**, 0 violation, best_quality 0,82. 21 tests + smoke `p150` 8/8, pytest **758/758 (+1 skip)** (+21 ; 737 avant), ruff clean, axe symbolique 0→1 (1ʳᵉ brique du dessin), global ~80,9 %. Veille J+13 (5 axes) → **0 combo externe** (Bevy 0.16/0.18, multi-agent LLM tier-2, ML-KEM-768, Neo4j vecteurs, mémoire agent LightMem/Memory3 — tous gated cargo/LLM/endpoint) ; combo interne C1×catalogue porté par l'archéochimie de l'ocre. Détail veille : [`docs/veille/2026-06-23_VEILLE_ochre_grinding.md`](docs/veille/2026-06-23_VEILLE_ochre_grinding.md). **Antérieur J+12 run #3 — Cap. C17 `iron_bloomery`** (bas-fourneau du fer) : 17ᵉ capacité agent et la **2ᵉ transformation MÉTALLURGIQUE** (après C13 cuivre) — le **seuil de l'âge du fer**. **RÉALISE** le `reaches_iron_bloomery_temp` que C12 exposait et que C13 différait explicitement. **1ᵉʳ retour fire-based depuis C13** (après 3 capacités non-fire C14/C15/C16) → **D9 redémarre à 1** (alternance honorée, cf. `R-J9r2-1`) ; **2ᵉ mutation** de l'arc (après `smelt_at` C13) → ouvre [`crates/MUTATION-FRONTIER.md`](native/world-engine/crates/MUTATION-FRONTIER.md) (R-J9-2 / R-J9r2-2, **D10 documenté/gelé** : un seul point de mutation `geo.mine_at`). **COMPOSE** C12 `forced_draught` (four ≥1200 °C **paroi réfractaire**) × C1 `surface_mineralization` (gossan, le « **chapeau de fer** ») × catalogue (`yields_per_kg_ore["Fe"]`, `category`) ; réutilise le seuil `fd.IRON_BLOOMERY_TEMP_C` ; **aucun nouveau tell** (D8 par composition, **11ᵉ** ; `PY_TO_RUST` reste 15, hors glob `*_outcrop.py`). **Physique calculée** (veille archéométrie bas-fourneau 2026-06-22) : réduction à l'état **SOLIDE** — le fer ne **FOND jamais** (1538 °C hors d'atteinte du four ≤1400 °C → **loupe** spongieuse à **marteler** ; `is_solid_bloom`/`requires_forging` toujours True, `furnace_reaches_iron_melt` toujours False, fonte/haut-fourneau différés) ; oxyde (hématite/magnétite) → réduction directe, fer **sain** ; pyrite (sulfure) → **griller** ~590 °C d'abord, fer **red-short** ; rendement monte avec la surchauffe, **plafonné < 1** (la scorie de **fayalite** `Fe₂SiO₄` garde du Fe). **Mensonge rendu visible #8** (chapeau de fer polyminéral) : le **même** tell gossan coiffe l'**oxyde** (fer sain), le **sulfure** pyrite (fer red-short) ET le **non-fer** galène/sphalérite (plomb/zinc → **0 fer**) — inversion **à 5 voies** (vs binaire C13). `bloom_at` **consomme** le minerai (réemploi `geo.mine_at`) & rend la loupe **exactement** promise par l'oracle (« le monde ne ment jamais » au sens FORT) ; `bloom_preview` non mutant. Monde réel seed `0x42` ancré **déterministe sans injection** : **78/144 sites = 47 hématite (oxyde) + 31 pyrite (sulfure red-short)**, 0 violation. 23 tests + smoke `p149` 8/8, pytest **737/737** (1 skip ; 714 avant C17), ruff clean, métallurgie 81→82, global ~80,7 → ~80,8 %. Veille J+12 r#3 → **0 combo externe** (Bevy 0.18 / multi-agent LLM / ML-KEM tous gated) ; combo interne C12×C1×catalogue porté par l'archéométrie. Détail : [`AUDIT-DELTA-2026-06-22-run3.md`](native/world-engine/AUDIT-DELTA-2026-06-22-run3.md), veille [`2026-06-22_VEILLE_iron_bloomery.md`](docs/veille/2026-06-22_VEILLE_iron_bloomery.md). **Antérieur J+12 run #1 — Cap. C16 `food_curing`** (salaison / conservation par le sel) : 16ᵉ capacité agent et la **1ʳᵉ capacité qui CONSOMME le PRODUIT d'une capacité précédente** — le **sel** de C15. **Exécute la reco `R-J9r2-3 (a)` de l'audit J+9 run #2** (« le sel rendu perceptible ouvre la conservation : salaison de la viande/poisson → autonomie alimentaire »). C15 a rendu le sel récoltable ; C16 expose la **vérité physique** de ce qu'on en fait — saler **arrête la pourriture** → réserve → surplus → sédentarité → commerce (« or blanc »). **3ᵉ capacité non-fire consécutive** (après C14 *ramasser* + C15 *sécher au soleil*) : l'alternance anti-treadmill tient, D9 (chaîne fire-based) reste à 0. **CONSOMME C15** (`salt_evaporation.best_saltpan_near` → le sel d'un marais salant récoltable) **× le champ macro de température** (climat, comme C14/C15) et **réutilise la lecture climat de C15** (`se._resolve_anchor`/`se._climate_at` — SSOT, 0 dérive). **Physique de l'activité de l'eau (a_w)** : le sel abaisse a_w par osmose, plancher **0,75** (saumure NaCl saturée, fait FIPS d'hygrométrie) ; croissance microbienne ∝ `((a_w−0,60)/(0,99−0,60))^5 × Q10^((T−25)/10)` (Q10=2,5) ; `shelf_life = SHELF_BASE / croissance`. Viande maigre fraîche à 25 °C → ~1,5 j ; salée à saturation (a_w 0,75, ~0,27 kg sel/kg) → **~181 j** (dynamique réelle du salaison ~100×). **Mensonge rendu visible #7** (après obsidienne C8, kaolin C9, cuivre C13, arène C14, lagune humide C15) : l'aliment **le plus appétissant** — la chair **fraîche**, rouge vif — est le **plus périssable** ; « frais = meilleur » est le mensonge, la fraîcheur se paie en pourriture. **Compromis émergent** : `palatability` et `nutrient_retention` **baissent** avec la dose → l'agent arbitre attrait↔conservation. **Sans marais salant à portée → dose 0 → l'aliment reste frais/périssable** (réciproque honnête : pas de sel, pas de conservation). **N'introduit AUCUN nouveau tell** (pas de `_PROFILE`, `PY_TO_RUST` inchangé à 15, hors glob `*_outcrop.py`) : garde-fou D8 **par composition** (10ᵉ après C7→C15), `test_introduces_no_new_tell`. `cure_food_at`/`achievable_cure_near` non mutants (D10 gelé). 1ʳᵉ avancée de l'**axe 3 (écosystème vivant / alimentation)**, immobile depuis J+0. Monde Genesis réel seed `0x5A17` (« SALT », même côte aride que C15) ancré **déterministe** sur la cellule saline la plus évaporative (sans injection) : **144/144 marais salants permettent la conservation** (best_shelf ~214 j, classe CURED), 0 violation. 26 tests + smoke `p148` 8/8, pytest **737 passed / 1 skip** (+26), ruff clean, global ~80,7 %. **Veille J+12** → **0 combo externe intégrable** (multi-agent civ Project Sid / AgentSociety / AIvilization, Bevy 0.18, ML-KEM CVE-2026-22705 tous gated cargo / LLM tier-2) ; signal neuf = **Deterministic Simulation Testing** (QCon / FOSDEM 2026) → backlog devtools. Antérieur J+9 run #2 — **Cap. C15 `salt_evaporation`** : 15ᵉ capacité agent et le **8ᵉ OPÉRATEUR ORTHOGONAL** — *sécher au soleil* (solar evaporation). **Réponse à la reco `R-J9-1` (audit J+9)** : « choisir le 8ᵉ opérateur **avant** de revenir au feu » (alternance anti-treadmill). 8ᵉ verbe primitif **non thermique** : laisser une saumure s'évaporer au soleil → **sel** (la chaleur vient du soleil, pas du foyer) ; 3ᵉ branche de la liste `R-J9-1` (eau bouillante / fermentation / **séchage solaire** / levier). Le **sel solaire** (marais salants, sabkhas, salars) conserve la nourriture et structure le commerce néolithique. **CONSOMME C3** (`water_potability` — salinité) **× l'aridité de Köppen** (`koeppen_grid._p_thresh` = `20·T+280`, SSOT « B aride » réutilisée *verbatim*) : net-évaporatif ssi `precip < p_thresh`. **INVERSION EXACTE de C3** : `MIN_BRINE_PPT == POTABLE_MAX_PPT` (3 ppt) — « trop salée pour boire » = « assez salée pour récolter ». Physique : `salt_yield = net_evap_mm × 1e-3 × salinity_ppt` (mer 35 ppt sous ~700 mm déficit → ~24,5 kg/m²/an). **Mensonge rendu visible #6** : saumure **identique** (35 ppt) en climat humide → 0 croûte (`SALINE_LAGOON`) ; en climat aride → sel (`SALAR`) — c'est le bilan évaporatif qui décide, pas la salinité. **Aucun nouveau tell** (D8 par composition, 9ᵉ ; `PY_TO_RUST` reste 15, hors glob `*_outcrop.py`). `harvest_salt_at` non mutant (D10 gelé, cf. R-J9-2). Monde réel seed `0x5A17` ancré **déterministe** sur la cellule saline la plus évaporative (argmax aridité, sans injection) : **144/144 marais salants récoltables**, 0 violation. 1ʳᵉ exploitation agent de l'**aridité climatique** (axe 2). 18 tests + smoke `p147` 8/8, pytest **688/688**, ruff clean, global ~80,6 %. Veille J+9 inchangée → **0 combo intégrable** (Emergence World / Bevy 0.18 / ML-KEM tous gated). Antérieur J+9 (run #1) — **Cap. C14 `cryoclasty`** : 14ᵉ capacité agent et le **7ᵉ OPÉRATEUR ORTHOGONAL** — *ramasser* (gather). **Réponse au verrou P0 `R-J8-1` (audit J+8)** : rompt la chaîne de **7 capacités fire-based consécutives** (C7→C13) qui escaladait D9 — 1ʳᵉ capacité non-fire depuis C6. Le gel (FCI Walder & Hallet, Wave 50) fragmente le socle en **gélifracts** qui gisent **en surface** (`collect_depth_m == 0`) → l'agent **ramasse** (ni percussion C2, ni feu C7). **1ʳᵉ consommation agent de l'observateur Wave 50** (jamais vu jusqu'ici → ferme la dette transparence R-J4-1). **Compose** Wave 50 (champ de gel macro) × C2 `lithic_outcrop._PROFILE`, **aucun nouveau tell** (D8 par composition, 8ᵉ ; `PY_TO_RUST` reste 15, hors glob `*_outcrop.py`). **Mensonge rendu visible #5** : `clast_quality = base(C2) × frost_response(fabric)` — versant froid+raide sur granite → **arène stérile** (gruss, désagrégation granulaire) ; sur obsidienne/silex → éclats prêts ; « froid+raide » ≠ bonne pierre, c'est le *fabric* qui décide. `gather_at` non mutant (D10 gelé). Monde réel seed `0xB0` ancré **déterministe** sur la cellule argmax-FCI (sans injection) : **144/144 champs = 139 taillables + 5 stériles `shale`**, 0 violation. 17 tests + smoke `p146` 8/8, pytest **670/670**, ruff clean, géologie 78→79. Aussi : R-J8-2 (nettoyage `genesis-engine/` orphelin + `err{1,2}.txt`) + R-J8-3 (`NEXT-LEVEL-AUDIT.md §0`) fermées ; veille J+9 → **0 combo intégrable** (Emergence World 2606.08367 + Bevy 0.18 + ML-KEM CVE-2026-22705 tous gated cargo/LLM → backlog P5). Antérieur J+8 run #2 — **Cap. C13 `copper_smelting`** : 13ᵉ capacité agent, la **4ᵉ transformation et la 1ʳᵉ MÉTALLURGIQUE** — le **seuil chalcolithique, le premier métal**. RÉALISE la promesse différée de C12 (`would_smelt_copper_here`) : `smelt_at` **consomme** réellement le minerai (mutation via `geo.mine_at`) et **rend** un bouton de cuivre + scorie, exactement comme l'oracle s'y engage (« le monde ne ment jamais » au sens FORT). **Mensonge rendu visible #4** : le même tell vert C1 couvre le cuivre **natif** (fonte directe, facile) ET la **chalcopyrite** (sulfure → **griller** ~590 °C avant, sinon scorie seule). Par composition C12×C1 + réemploi seuil `fd.COPPER_SMELT_TEMP_C` + rendement catalogue, **aucun nouveau tell** (garde-fou D8 par composition, 7ᵉ). Monde 0xBEEF : 21 sites de fonte émergents (18 natifs + 3 sulfures) **sans injection**. Antérieur J+8 run #1 : Cap. C12 `forced_draught` (2ᵉ apparatus, tirage forcé soufflet+charbon, vitrification + ouverture métallurgie, C11×C1) ; J+7 run #2 : Cap. C11 `kiln_draft` (1ᵉʳ apparatus) ; J+7 run #1 : Cap. C10 `lime_burning` ; J+6 run #2 : Cap. C9 `ceramic_firing` ; Cap. C8 `lithic_tempering` + `crates/STATUS.md` (R1) ; J+5 : Cap. C7 `fire_ignition`, ADR-0008 + garde-fou D8 — détail dans [`PROJECT-STATUS.md`](PROJECT-STATUS.md)).
