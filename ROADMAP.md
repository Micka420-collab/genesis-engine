...
| **Fill–Spill–Merge** (flow routing en hierarchie de depressions) | World (hydrologie, Wave 64 `river_discharge`) | Barnes/Callaghan, NSF par.10263903 + Springer 2025 (veille 2026-06-30) | lacs endoreiques emergents : les cuvettes/depressions piegent le flux → lacs au lieu d'un routage purement descendant ; reste **physique pure** (pas un arbre tech) | **Fondation livree 2026-07-01 — Wave 66** `engine.lake_hydrology` (Priority-Flood, Barnes 2014) : observateur read-only **additif** qui livre la depression-storage + les lacs endoreiques a extension maximale (16 lacs, cross-check vs pits D8). **Remplissage livre 2026-07-02 — Wave 67** `engine.fill_spill_merge` (FSM, Barnes/Callaghan/Wickert 2021) : verse l'apport route de Wave 64 (`route_runoff`) dans ces contenants → lacs **partiels**, **playas** (bassin terminal affame, sel concentre ∝ 1/remplissage), full-spill ; **additif read-only** final (reutilise Wave 66 + Wave 64 en lecture, ne mute AUCUN code teste — garde-fou respecte, suite pytest complete verte) ; volume-fini ne deborde jamais la capacite, surface partielle plane, spectre monotone selon la fenetre d'apport ; 18 tests + smoke p177 8/8. **Reste Wave 68** : la cascade inter-bassins (router l'`overflow_m3` d'une cuvette pleine vers la cuvette avale → sequence spill→merge dynamique le long de la hierarchie). |
