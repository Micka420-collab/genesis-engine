...
| **Tests** | `cd runtime && python -m pytest tests/ -q` → **157 tests OK** |
...
```bash
cd runtime && python -m pytest tests/ -q    # 157 tests
...
5. **157 tests pytest** doivent rester verts après toute modification.
...
```text
Genesis Engine · ZERO PRE-SCRIPT · L0 earth_laws → L4 civilization · observer http://127.0.0.1:8090/ · wire_emergence_v2 · 157 tests · réalisme Terre ~76%
```
