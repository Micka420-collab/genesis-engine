...
**Tests :** `pytest runtime/tests` — **426** tests · smokes **p72–p134** (+ `p87` observer) dans `validate-all`.  
...
**Score global : ~79,3 %** (moyenne 7 dimensions = (80+75+73+77+82+86+82)/7 = 79,3 % — Sociétés/agents 76→77 via **Cap. C2 affleurements de pierre taillable** : première _capacité_ de découverte d'**outil lithique** émergente — la pierre taillée, technologie plus fondamentale que le minerai ; antérieur 79,1 % Cap. C1 géologie 74→75 indices de surface minéralisée). **Objectif cible : 80 %**. _NB (audit §3) : ce score mesure le réalisme **observé** ; le sous-score « capacité moteur Rust » (A3/A4/A5/B1–B8) reste à **0/7** — non touché par cette capacité Python.
