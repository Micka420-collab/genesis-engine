...
**Score global : ~78 %** (moyenne 7 dimensions, recalcul **78,4 %** après Wave 57 — géologie 68→70, hydrologie 72→73). **Objectif cible : 80 %** — voir explication des anciens chiffres (68 / 74 / 80) dans la roadmap.

| Dimension | % | Piste principale |
|-----------|---|------------------|
| Climat / biomes | 80 | GraphCast-lite + colonne 3D + circulation L1 + vent 2D |
| Géologie / relief | 70 | Tectonique live, stratigraphie + datation relative + absolue (Wave 51) + cryoclastie (Wave 50) + compaction diagénétique (Wave 54) + géotherme/faciès métamorphiques (Wave 56) ; **Wave 57 lit mobile / transport sédimentaire Exner** (capacité stream-power sur le débit LTI, érosion/dépôt, fermeture de masse exacte ΣE=ΣD+export) |
