"""Genesis Engine simulation loop (Phase 5).

Phase 5 changes vs. Phase 4:
  * Vectorised thermal update — chunks bucketed once per tick instead of
    per-agent lookup.
  * Group bookkeeping — emit ``group_dissolved`` when live membership
    falls below the formation threshold; reclaims orphaned agents so they
    can rejoin new clusters.
  * Lexicon signatures use the process-stable BLAKE2 hash (was Python's
    randomised hash()).
"""
...
class Simulation:
    ...
    def step(self) -> SimStats:
        ...
        for row in alive_idx:
            row = int(row)
            obs = perceive(self.agents, row, self.streamer, grid=self._grid)
    ...
    def _tick_thermal(self) -> None:
        ...
        Bucket alive agents by chunk coord so chunk lookup and weather_at
        are amortised over many agents instead of run per-agent.  At 200
        agents this saves ~50-80 ms/tick vs. the per-row loop.
        """
        n = self.agents.n_active
        if n == 0:
            return
        ...
        for (cx, cy), rows in unique_coords.items():
            ...
            therm = self.agents.thermal[rows_arr].copy()
            for i in range(rows_arr.size):
                ...
                therm[i] = float(np.clip(
                    float(therm[i]) + (comfort - 1.0) * 0.02 * accel * 0.001 - 0.005,
                    0.0, 1.5))
